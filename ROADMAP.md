...
- [ ] **E4: Repo scaffolding** — `biscuit generate` emits a complete repo that builds and releases. → [Generated repo structure](PRD.md#generated-repo-structure), [Distribution](PRD.md#distribution), [Regeneration safety](PRD.md#regeneration-safety)
    ...
    - [ ] Template `install.sh` (proven on biscuit's own, adapted from opencode's, MIT) into generated repos as a third distribution channel alongside brew/npm — installs to `~/.{binary}/bin`, `--channel`/`--version` flags mirroring `upgrade`'s. Template the bare-invocation TTY splash (proven on biscuit's own root command) into generated CLIs: name/description from the spec via the IR (same `biscuit.yaml`-overrides-first rule as the Makefile), figlet-rendered logo for an arbitrary binary name (`common-nighthawk/go-figure` — deterministic, no runtime dependency, avoids hand-drawn glyphs that only work for one fixed word), piped/non-TTY always falls through to cobra's normal help.
