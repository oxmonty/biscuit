...
4. Add an `NPM_TOKEN` repo secret (granular automation token) for the bootstrap release, then configure npm trusted publishing (OIDC) for `biscuit-cli` and each `@oxmonty/biscuit-*` platform package against this repo's `release.yaml` workflow and delete the token.
