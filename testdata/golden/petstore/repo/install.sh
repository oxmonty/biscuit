...
# petstore installer — a third channel alongside Homebrew and npm, for
# anyone who doesn't want either. Structure, PATH-detection, and shell-config
# logic adapted from opencode's install script (MIT): https://github.com/sst/opencode
set -euo pipefail

REPO="oxmonty/petstore-cli"
INSTALL_DIR="$HOME/.petstore/bin"
...
Options:
    ...
    -v, --version <version>    Install a specific version (e.g. 1.2.3)
    -c, --channel <channel>    stable or next (default: next)
...
Examples:
    curl -fsSL https://raw.githubusercontent.com/oxmonty/petstore-cli/main/install.sh | bash
    curl -fsSL https://raw.githubusercontent.com/oxmonty/petstore-cli/main/install.sh | bash -s -- --version 1.2.3
EOF
}
...
[[ "$os" == "windows" ]] && binary_name="petstore.exe"
...
if [[ -n "$binary_path" ]]; then
    ...
    filename="petstore_${requested_version}_${os}_${arch}.${archive_ext}"
    base_url="https://github.com/$REPO/releases/download/v${requested_version}"
    url="$base_url/${filename}"

    echo -e "${MUTED}Downloading${NC} petstore ${requested_version} ${MUTED}(${os}/${arch})${NC}"
    tmpdir=$(mktemp -d)
...
if [[ "$no_modify_path" != "true" ]] && [[ ":$PATH:" != *":$INSTALL_DIR:"* ]]; then
    ...
    if [[ -n "$config_file" ]]; then
        ...
        if ! grep -qF "$INSTALL_DIR" "$config_file" 2>/dev/null; then
            { echo ""; echo "# petstore"; echo "$line"; } >> "$config_file"
        fi
        echo -e "${MUTED}Successfully added ${NC}petstore ${MUTED}to \$PATH in ${NC}$config_file"
    else
...
echo -e ""
echo -e "${MUTED}installed petstore — run${NC} petstore --help"
