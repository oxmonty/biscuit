...
- `biscuit.yaml` sidecar config for overrides (resource names, aliases, hidden endpoints, pagination hints, distribution), schema-validated on load: unknown keys rejected with precise errors, a `version` key for forward migration — the config drives codegen, so a malformed config must fail loudly, never mis-generate. Open question: also honor in-spec `x-biscuit-*` extensions (ogen's model) or standard OpenAPI Overlays — sidecar suits users who don't control the spec; extensions suit spec owners.
...
- **Quickstart in help** (designed, not yet built — reverted back out of biscuit's root command after a same-day prototype so E4 does it once, deliberately): `root.Long` would carry the spec-derived description plus quickstart lines and docs link, plain text (no color, no logo) — cobra's own template puts it above `Usage:` on `{binary} help`, `--help`, `-h`, *and* bare invocation for free, since a root command with no `Run` already falls back to `cmd.Help()`. No custom Run, no TTY detection, no new dependency. (A TTY-gated colored splash with a figlet logo was tried first and simplified away in favor of this plain-everywhere approach — the lesson carries forward even though the code doesn't.)
- Opt-in via config:
...
- `pb33f/libopenapi` (spec — see Open questions), `daveshanley/vacuum` (spec lint/doctor), `spf13/cobra` (commands), `tidwall/gjson` (transforms), `charmbracelet/bubbletea|lipgloss|glamour` (chat TUI + explore), `modelcontextprotocol/go-sdk` (MCP), `zalando/go-keyring` (auth), GoReleaser + release-please (release), `text/template` + `embed.FS` (codegen)
...
|**restish**|OSS prior art on OpenAPI→CLI mapping (runtime model); its author's mapping writeup feeds the E3 heuristics.|
