...
- **Quickstart in help**: `root.Long` carries the spec-derived description plus quickstart lines and docs link, plain text (no color, no logo) — cobra's own template puts it above `Usage:` on `{binary} help`, `--help`, `-h`, *and* bare invocation for free, since a root command with no `Run` already falls back to `cmd.Help()`. Proven on biscuit's own root command; no custom Run, no TTY detection, no new dependency. (An earlier TTY-gated colored splash with a figlet logo was built and then deliberately simplified away in favor of this — one help surface, everywhere, always plain.)
- Opt-in via config:
