...
- **Welcome splash**: bare `{binary}` on a TTY prints a colored banner (name, spec-derived description, quickstart, docs link) instead of cobra's default help; piped/non-TTY invocations always fall through to plain help, so scripts see nothing new. Proven on biscuit's own root command. Name is figlet-rendered (`common-nighthawk/go-figure`, deterministic, no runtime dependency) rather than hand-drawn block art, since generated binary names are arbitrary user data, not a name we control once (biscuit's own splash uses a fixed figlet render for the same reason — consistency over one-off bespoke art).
- Opt-in via config:
