...
- Reserved `internal/custom/` in output — emitted once, never overwritten. Overwrite protection is not contract protection: `internal/custom/` gets a defined stable surface it may depend on (the generated client's exported API), and compile-the-output CI includes a repo with a representative custom file so a regeneration that breaks the contract fails in biscuit's CI, not the user's. Custom cobra *commands* currently require stripping root.go's marker (owning the file); the planned hook — root calling an emit-once `custom.RegisterCommands(root, f)` — ships with E9 so custom commands survive automated update-PR regenerations.
...
Reference targets: Fern publishes its own CLI generator output at [fern-api/petstore-cli](https://github.com/fern-api/petstore-cli) (Rust — irrelevant to tiers 1–2, which are black-box by design) on the same petstore spec as our easy rung. Speakeasy [ships a CLI generator](https://www.speakeasy.com/docs/cli-generation) (beta since 2026, `speakeasy quickstart --target cli` → Go/Cobra wrapping its generated Go SDK) but publishes no reference output — [their examples repo](https://github.com/speakeasy-api/examples) still covers only SDKs, Terraform providers, tests, and MCP servers, so there is nothing public to chart. Same blocker as Fern's non-petstore output: generating a comparison repo needs a gated account (beta access is request-only), which is the spike task below. Until then Speakeasy is footnoted as ungenerated, **not** charted as zero — a real competitor missing from the chart for access reasons, not capability. Customer CLIs were surveyed (2026-07) for generated alternatives: every public CLI from Fern/Speakeasy customers (auth0-cli, twilio-cli, Kong/deck, airbyte abctl, vercel, cloudflare wrangler) is hand-written — no `.fernignore`/`.speakeasy` markers — and a global `.fernignore` code search surfaces only SDKs plus petstore-cli itself. Fern's CLI generator is too new to have public customer output; petstore-cli is the reference until that changes. Account-gated generation of further comparison repos (Fern on train-travel, Speakeasy on the petstore rung) is a spike task. Scores are dated and SHA-paired like the parity number.
...
- **Pagination**: transparent walking — a matched operation fetches every page like Stainless SDKs and their CLI, streaming pages as they arrive (JSONL when piped); `--max-pages N` bounds the walk. Detection is scheme-based (Stainless's model): named schemes under `pagination:` in `biscuit.yaml` (cursor/offset/page_number types with request-param and response-field mappings), auto-matched against operation shapes, plus a built-in scheme library covering the dominant wire conventions (openai/Stripe-style cursor, classic `page`/`offset`) so unconfigured specs paginate out of the box. Matching is conservative: exact word-boundary vocabulary (never substring), corroboration required from both request (a pagination param) and response (items array + next-page signal); ambiguity → no pagination plus a doctor advisory suggesting a declared scheme; per-operation hints disambiguate or disable. Streaming endpoints are wired to paging tools.
...
```
{binary} mcp serve --transport stdio|http [--toolset name]
...
- **Toolsets** — named allowlists of operations/tags in `biscuit.yaml` (`toolsets: {public: [...], admin: [...]}`); `mcp serve --toolset public` filters what `tools/list` advertises and `tools/call` accepts at serve time. Distinct from `operations: ignore`, which removes an operation from generation entirely — a toolset scopes one serving surface while the CLI keeps the full command tree. Default toolsets are seeded deterministically from what specs already declare (tags, `x-internal`), and the sidecar recipe above defaults to the public toolset so admin operations never reach `/mcp` — the toggle is the trust-boundary control that makes the sidecar responsible advice. This is the single-API seed of the multi-API conviction that curation is the product ([Future](#future-multi-api-toolsets-and-spec-acquisition)); LLM-*proposed* toolsets are the spec-acquisition skill's territory, never a serve-time decision.
...
│   └── golden/                 # committed golden repos: petstore + galaxy (galaxy carries the representative internal/custom/ file), -update flag; larger ladder specs assert a FilePlan content hash instead of committed bytes
...
The generated client lands with its final exported shape in E4 — one method per operation over plain `net/http` — and E5 layers auth, retries, pagination, and output semantics behind those same signatures. That exported surface is the `internal/custom/` contract from first emission, which is what makes the compile-the-output custom-file gate meaningful from E4 onward.

### Target languages

Go through v0.2; **Rust** is the second target (E14), with TypeScript and Python scoped from what building it teaches.

The seam is `internal/ir`, which is language-neutral by construction and stays that way: it carries wire-shape data only — `Flag.Type` is the JSON Schema set (`string|integer|number|boolean|json`), `Flag.Param`/`Flag.BodyPath` are wire names, security is `type/scheme/in/param` over an OR-list of AND-sets. Every Go-ism (`RegCall`, `DefLit`, `GetCall`, `WireExpr`, `goExported`, `pkgName`) is computed in `internal/render`, below the seam. A second target is a second renderer over the same IR, not a fork. Treat a Go-shaped field reaching `internal/ir` as a defect: it is the one change that would make a second target expensive.

Rust goes first because it is the only other target that preserves the [Distribution](#distribution) spine — a cross-compiled, dependency-free static binary. TypeScript and Python cannot deliver that without a second distribution design (interpreter runtime, or Bun/Deno/PyInstaller bundling), which is why they are scoped after Rust rather than alongside it. Ordering evidence and the cost model are in the [Open questions](#open-questions) entry.
...
**Quality gates (every PR, all required):** lint (`golangci-lint`), unit tests, golden-output tests (`testdata/golden`, `-update` flag, pinned Go toolchain), the easy/medium integration rungs, **compile-the-output** (`go build ./...` on the committed golden repos — petstore + galaxy, galaxy carrying the representative `internal/custom/` file — plus freshly generated output for the larger ladder specs; the gate that matters most), and the generation benchmark tracked for regressions. Post-E6: `biscuit bench` parity vs openai-cli runs on PRs touching mapping or templates, with the number surfaced in the PR.
...
**Release speed:** the release job carries its own cross-compile build cache (`release-go-` key with a `restore-keys` prefix fallback) — the ci workflow's cache collides on the same go.sum key and only ever holds linux/amd64 objects, so without the dedicated key every release rebuilds all targets cold. Warm releases recompile only changed dependencies; within the job, goreleaser already parallelizes across cores. The cache's real ceiling is GitHub's 7-day idle eviction: releases spaced more than a week apart always start cold (~28 min for the 7-target build; v0.1.0-alpha.7 measured it) — a weekly scheduled cache-refresh job is the upgrade path if the cadence stays slower than the eviction window. Templated into generated repos' release workflow by default — same 7-target build, same win. goreleaser Pro's `--split` per-platform matrix stays off the default path (a paid dependency generated repos shouldn't inherit); revisit only if warm-cache releases are still too slow.
...
- **Speakeasy** — the funded incumbent, and a direct CLI competitor, not just an SDK tool: it generates [Go/Cobra CLIs](https://www.speakeasy.com/docs/cli-generation) (wrapping its generated Go SDK, GoReleaser included; beta, request-only access) and, separately, MCP servers from OpenAPI. The openness gap is the wedge: account-gated SaaS (free tier ~one SDK/CLI up to 50 methods, paid per-language), and while its generator CLI runs air-gapped it still needs a license — not self-hostable.
...
- **Resolved (2026-07):** pagination — transparent walking bounded by `--max-pages` (explicit `--all` opt-in rejected as the default), detected via Stainless's scheme model: `pagination:` schemes in `biscuit.yaml` auto-matched by request/response shape, plus a built-in convention library (surveyed from Stripe/OpenAI/GitHub/Slack/AWS/Google et al.) so unconfigured specs walk. Evidence: Stainless's own CLI paginates transparently (TTY → lazy-loading pager, piped → stream) and its SDKs declare schemes in `stainless.yml` matched automatically ([pagination docs](https://www.stainless.com/docs/sdks/configure/pagination/), [CLI changelog](https://www.stainless.com/changelog/cli-pagination)). False-positive guard: exact word-boundary vocabulary with both-sides corroboration; ambiguity resolves to no pagination plus a doctor advisory.
- **Resolved (2026-07):** E5 mock delivery — the spec-generated mock (routes + schema-valid canned responses + request recording) renders into generated repos as test code alongside templated smoke tests, and biscuit's own e2e runs the repo's smoke machinery, replacing E4's echo server; E6's ladder and parity bench reuse the same generator. Biscuit-internal-only was rejected: generated repos can't import biscuit, so the harness-only shape would build the mock generator without its hardest consumer and ship E5 without self-testing repos.
- **Resolved (2026-07):** E4 client scope — generated commands ship a naive HTTP client: per-operation methods with their final exported signatures making plain `net/http` calls; E5 layers auth, retries, pagination, and output control behind those signatures without changing them. Stub-bodies-only was rejected (compile-the-output's `internal/custom/` gate needs a real exported surface, and a CLI whose every command errors demos nothing); deferring the contract to E5 was rejected (it ships E4's regeneration-safety story half-done).
- **Resolved (2026-07):** repo creation for generated output — delegated to `gh`, no native `--push`/`--url` flag on `biscuit generate`. The generated SETUP.md opens with the `gh repo create --source=. --push` one-liner and the templated setup-publishing skill creates and pushes the repo itself when no origin exists. A native push flow was rejected: `gh repo create` already does it in one authenticated command, wrapping it would pull auth handling, visibility/org decisions, and an irreversible publish action into a tool whose generation contract is deliberately pure, and "on GitHub" isn't the real gap anyway — the publishing setup (tap, PAT, org settings) is, which SETUP.md/the skill own. Revisit only if E9's update pipeline needs biscuit itself to touch remotes; remote `spec.source` fetching stays E9 scope.
- **Resolved (2026-07):** golden corpus — committed golden repos for petstore (easy rung, the parity spec) and galaxy (multi-auth, uploads, a real cycle; carries the representative `internal/custom/` file). openai/stripe and the rest of the ladder generate and `go build` in CI every run but pin determinism via a FilePlan content hash, no committed bytes — whole-ladder commits were rejected for tens-of-MB repo bloat and unreviewable template-change diffs.
...
- **Resolved (2026-08):** target languages for generated CLIs — Go through v0.2, then **Rust as the single second target** (E14), with TypeScript and Python scoped from what Rust teaches; no `--language` flag until a second renderer exists to put behind one. Achievability is settled, not assumed: a throwaway TypeScript renderer built over the real `spec.Load → mapping.Map` pipeline found **0 IR gaps** — 612 LOC (412 Go renderer + 222 templates) produced a `tsc --noEmit`-clean commander.js CLI whose commands ran against a live server with correct path substitution, query params, JSON bodies, and integer coercion. The IR needed no change because it carries wire shapes only; TS needed *less* work than Go on `Flag.Default` (JSON literals are already JS literals) and reused the security OR-of-AND-sets logic line-for-line. Cost is the reason for the ordering, and it is downstream of the renderer: 2,853 of 3,675 template LOC (78%) and 991 of 1,166 `internal/render` LOC (85%) are target-specific, plus ~2,000 LOC of tests and 28 golden files asserting Go output — ~3,850 LOC per target, and the expensive parts are *logic*, not templating (`WalkPages` across cursor/offset/page × four next-kinds, multipart file parts, SSE streaming). The probe covered petstore only (1 resource, 3 verbs, 5 flags, no security schemes exercised) at roughly E4 scope. Rust wins the second slot on [Distribution](#distribution): it is the only other target keeping the cross-compiled dependency-free static binary the whole spine assumes (GoReleaser × 7 platforms, Homebrew cask, npm platform packages, `install.sh` with no runtime dependency, `upgrade`'s bare-binary self-swap) — TS and Python each need a second distribution design first. Doing all four at once was rejected (~11,500 LOC, and it multiplies the six epics naming Go, two of which — E7's MCP `go-sdk`, E8's Bubble Tea — are Go-bound by resolved entries above). Doing none was rejected as forfeiting the one axis competitors monetize. Market evidence that this is not urgent: no CLI generator in the field ships a language flag — [Speakeasy](https://www.speakeasy.com/docs/cli-generation) has 10 SDK languages but a Go/Cobra-only CLI, [Fern](https://buildwithfern.com/learn/cli-generator/get-started/overview) has 10+ SDK languages but a Rust-only CLI marketed as "a single statically linked binary (no runtime required)", Stainless's CLI was Go (its Rust/Swift work was SDK targets), restish is one shared Go binary. The asymmetry is structural: you *import* an SDK so its language must match yours, you *exec* a CLI across a process boundary where its language is invisible — per-language pricing is an SDK phenomenon. The one real counter-need, a team extending the generated repo in its own language, is already served by the `internal/custom/` contract. Spike code discarded; these numbers are the artifact.
