...
- **No telemetry, ever**: biscuit and generated CLIs phone home to nothing — no usage pings, no crash reporting, no update checks beyond the explicit `upgrade` command. A hard differentiator against account-gated generators, and table stakes for the self-hosted positioning; if usage insight is ever wanted, it arrives opt-in and loudly documented.
...
- Chat TUI substrate: port/reuse [pi](https://github.com/earendil-works/pi)'s TypeScript UI against the MCP server and chat endpoint (real code reused, but needs a node runtime or a companion npm package — breaks the single-static-binary spine) vs recreating it in Go/Bubble Tea (single binary preserved, full rewrite cost) — settled by a spike that opens E8.
